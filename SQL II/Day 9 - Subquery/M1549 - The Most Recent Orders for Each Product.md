# M1549 - The Most Recent Orders for Each Product
...
- We first get all the lastest order using `MAX(order_date)`, then we `LEFT JOIN` the `product` with the lastest date for matching
